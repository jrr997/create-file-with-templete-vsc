...
export function activate(context: vscode.ExtensionContext) {
  ...
  let disposable = vscode.workspace.onDidCreateFiles(async (e) => {
    ...
    for (const uri of e.files) {
      ...
      const isExcluded = [...(excludes ?? []), ...defaultExcludes].some((pattern) => {
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeConfiguration((e) => {
      if (e.affectsConfiguration('CreateFile.templateDir')) {
        const newTemplateDir = vscode.workspace.getConfiguration('CreateFile').get('templateDir') as string;
        if (newTemplateDir !== templateDir) {
          templateDir = newTemplateDir;
        }
      }
      if (e.affectsConfiguration('CreateFile.template')) {
        const newTemplateConfig = vscode.workspace.getConfiguration('CreateFile').get('template') as { [key: string]: string[]};
          templateConfig = newTemplateConfig;
      }
      if (e.affectsConfiguration('CreateFile.excludes')) {
        const newExcludes = vscode.workspace.getConfiguration('CreateFile').get('excludes') as string[];
